namespace LewdFilter.Domain.Constants;

public static class WeaponBaseTypeConstants
{
    public static readonly string[] Bow =
    [
        "Crude Bow",
        "Shortbow",
        "Warden Bow",
        "Recurve Bow",
        "Composite Bow",
        "Dualstring Bow",
        "Cultist Bow",
        "Zealot Bow",
        "Artillery Bow",
        "Tribal Bow",
        "Twin Bow",
        "Adherent Bow",
        "Militant Bow",
        "Ironwood Shortbow",
        "Cavalry Bow",
        "Guardian Bow",
        "Warmonger Bow",
        "Gemini Bow",
        "Fanatic Bow",
    ];

    public static readonly string[] Crossbow =
    [
        "Makeshift Crossbow",
        "Tense Crossbow",
        "Sturdy Crossbow",
        "Varnished Crossbow",
        "Dyad Crossbow",
        "Alloy Crossbow",
        "Bombard Crossbow",
        "Construct Crossbow",
        "Blackfire Crossbow",
        "Robust Crossbow",
        "Piercing Crossbow",
        "Twin Crossbow",
        "Cannonade Crossbow",
        "Bleak Crossbow",
        "Trarthan Cannon",
        "Stout Crossbow",
        "Engraved Crossbow",
        "Desolate Crossbow",
        "Flexed Crossbow",
        "Elegant Crossbow",
        "Gemini Crossbow",
        "Siege Crossbow",
    ];

    public static readonly string[] OneHandedMace =
    [
        "Wooden Club",
        "Smithing Hammer",
        "Slim Mace",
        "Spiked Club",
        "Warpick",
        "Plated Mace",
        "Brigand Mace",
        "Construct Hammer",
        "Morning Star",
        "Jade Club",
        "Marching Mace",
        "Bandit Mace",
        "Structured Hammer",
        "Flanged Mace",
        "Crown Mace",
        "Brigand Mace",
        "Marauding Mace",
        "Molten Hammer",
        "Akoyan Club",
        "Strife Pick",
        "Fortified Hammer",
    ];

    public static readonly string[] Quarterstaff =
    [
        "Wrapped Quarterstaff",
        "Long Quarterstaff",
        "Gothic Quarterstaff",
        "Crackling Quarterstaff",
        "Crescent Quarterstaff",
        "Steelpoint Quarterstaff",
        "Slicing Quarterstaff",
        "Barrier Quarterstaff",
        "Hefty Quarterstaff",
        "Smooth Quarterstaff",
        "Waxing Quarterstaff",
        "Bladed Quarterstaff",
        "Guardian Quarterstaff",
        "Sinister Quarterstaff",
        "Lunar Quarterstaff",
        "Razor Quarterstaff",
        "Striking Quarterstaff",
        "Bolting Quarterstaff",
        "Dreaming Quarterstaff",
        "Aegis Quarterstaff",
        "Skullcrusher Quarterstaff",
    ];

    public static readonly string[] Quiver =
    [
        "Broadhead Quiver",
        "Fire Quiver",
        "Sacral Quiver",
        "Two-Point Quiver",
        "Blunt Quiver",
        "Toxic Quiver",
        "Serrated Quiver",
        "Primed Quiver",
        "Penetrating Quiver",
        "Volant Quiver",
        "Visceral Quiver",
    ];

    public static readonly string[] Sceptre =
    [
        "Rattling Sceptre",
        "Stoic Sceptre",
        "Omen Sceptre",
        "Shrine Sceptre",
        "Wrath Sceptre",
    ];

    public static readonly string[] Spear =
    [
        "Hardwood Spear",
        "Ironhead Spear",
        "Hunting Spear",
        "Winged Spear",
        "War Spear",
        "Forked Spear",
        "Barbed Spear",
        "Broad Spear",
        "Crossblade Spear",
        "Seaglass Spear",
        "Branched Spear",
        "Jagged Spear",
        "Massive Spear",
        "Helix Spear",
        "Orichalcum Spear",
        "Pronged Spear",
        "Spiked Spear",
        "Stalking Spear",
        "Akoyan Spear",
        "Flying Spear",
        "Grand Spear",
        "Soaring Spear",
        "Guardian Spear",
    ];

    public static readonly string[] Staff =
    [
        "Ashen Staff",
        "Gelid Staff",
        "Voltaic Staff",
        "Pyrophyte Staff",
        "Chiming Staff",
        "Reaping Staff",
        "Roaring Staff",
        "Sanctified Staff",
        "Paralysing Staff",
    ];

    public static readonly string[] TwoHandedMace =
    [
        "Felled Greatclub",
        "Oak Greathammer",
        "Forge Maul",
        "Studded Greatclub",
        "Cultist Greathammer",
        "Temple Maul",
        "Leaden Greathammer",
        "Crumbling Maul",
        "Pointed Maul",
        "Totemic Greatclub",
        "Solemn Maul",
        "Heavy Greathammer",
        "Disintegrating Maul",
        "Anvil Maul",
        "Sacred Maul",
        "Ironwood Greathammer",
        "Massive Greathammer",
        "Fanatic Greathammer",
        "Tawhoan Greatclub",
        "Ruination Maul",
    ];

    public static readonly string[] Wand =
    [
        "Withered Wand",
        "Attuned Wand",
        "Bone Wand",
        "Siphoning Wand",
        "Volatile Wand",
        "Galvanic Wand",
        "Dueling Wand",
    ];

    public static readonly IReadOnlyList<string> Talismans =
    [
        "Alpha Talisman",
        "Ashbark Talisman",
        "Changeling Talisman",
        "Cinderbark Talisman",
        "Condemned Talisman",
        "Cruel Talisman",
        "Fang Talisman",
        "Familial Talisman",
        "Frenzied Talisman",
        "Fungal Talisman",
        "Fury Talisman",
        "Howling Talisman",
        "Jade Talisman",
        "Lumbering Talisman",
        "Maji Talisman",
        "Nettle Talisman",
        "Primal Talisman",
        "Rabid Talisman",
        "Spiny Talisman",
        "Thunder Talisman",
        "Vicious Talisman",
        "Voltfang Talisman",
        "Wildwood Talisman",
        "Wingbeat Talisman",
    ];
}
